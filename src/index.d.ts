...
export class Channel {
  constructor(topic: string, params?: object | (() => object), socket?: Poshti);
  ...
  onClose(
    callback: (payload: any, ref: any, joinRef: any) => void | Promise<void>,
  ): number;
...
export type ParamsType<TUserRole extends UserRoleType> = {
  public_user: {};
  admin: { auth: string };
  end_user: { auth: string; channel: string };
}[TUserRole] & { pid: string };

export interface SocketConnectOption<TUserRole extends UserRoleType> {
  binaryType: BinaryType;
  userRole: TUserRole;
  params: ParamsType<TUserRole>;
...
export class Poshti<TUserRole extends UserRoleType> {
  constructor(opts?: Partial<SocketConnectOption<TUserRole>>);
  ...
  channel(channelName: string, chanParams?: object): Channel;
