...
import {
  ...
  DEFAULT_TIMEOUT,
  DEFAULT_VSN,
  POSHTI_CHANNEL_PREFIX,
  POSHTI_WSS_END_POINT,
  SOCKET_STATES,
  TRANSPORTS,
  USER_ROLES,
  WS_CLOSE_NORMAL,
 ...
 * @param {Function} [opts.transport] - The Websocket Transport, for example WebSocket.
 *
 ...
 *
 * TODO: check this place for necessitate
...
export default class Poshti {
  constructor(opts = {}) {
    ...
    this.heartbeatIntervalMs = opts.heartbeatIntervalMs || DEFAULT_HEARTBEAT;
    ...
    this.reconnectAfterMs = tries => {
      ...
      return [250, 500, 1000, 2000, 4000, 5000][tries - 1] || 30000;
    };
    ...
    this.userRole = opts.userRole || USER_ROLES.publicUser;
    if (this.userRole === USER_ROLES.admin) {
      delete this.params.pid;
    }
    this.endPoint = `${POSHTI_WSS_END_POINT}/${TRANSPORTS.websocket}`;
   ...
   */
  endPointURL() {
    let uri = URI.appendParams(this.endPoint, {
      ...this.params,
      vsn: this.vsn,
    });
   ...
   * @returns {Channel}
   */
  channel(channelName, chanParams = {}) {
    const topicTemplate = `${POSHTI_CHANNEL_PREFIX}:${this.pid}:${channelName}`;
    let chan = new Channel(topicTemplate, chanParams, this);
