...
export default class Channel {
   ...
   * @returns {Push}
   */
  push(topic, payload, timeout = this.timeout) {
    payload = payload || {};
    if (!this.joinedOnce) {
      throw new Error(
        `tried to push '${topic}' to '${this.topic}' before joining. Use channel.join() before pushing events`,
      );
    }
    let pushEvent = new Push(
      this,
      `${POSHTI_TOPIC_PREFIX}:${topic}`,
      function () {
        return payload;
      },
      timeout,
